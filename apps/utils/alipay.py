...
class AliPay(object):
    ...
    def sign_data(self, data):
        ...
        quoted_string = "&".join("{0}={1}".format(k, quote_plus(v)) for k, v in unsigned_items)  # 对签名进行处理
...
if __name__ == "__main__":
    ...
    alipay = AliPay(
        appid="",
        app_notify_url="http://127.0.0.1:8000/alipay/return/",     # 用户进入了支付页面，并没有支付直接关闭，异步接受接口
    ...
    url = alipay.direct_pay(
        ...
        total_amount=100,
        return_url="http://127.0.0.1:8000/alipay/return/"    # 有了这个参数，订单支付完成后跳转回商户页面
    )
